<template>
  <q-card class="no-shadow full-width bg-grey-1">
      <q-card-section class="bg-grey-3">
          <span class="text-h6">{{ $t('job.phraseSettings') }} </span>
      </q-card-section>
          ...
          <q-table :columns="columns" :rows="phraseList" row-key="id" selection="multiple" class="no-shadow"
              ...
              <template v-slot:header-cell-name="props">
                  <q-th :props="props" class="q-pa-none">
                      <div> {{ $t('phraseSettings.phraseID') }} </div>
                      <div> {{ $t('phraseSettings.phraseName') }} </div>
                  </q-th>
              </template>
      ...
      <addPhraseComponent ref="jobPhraseDrawer" />
      </q-card>
</template>
...
const jobPhraseDrawer = ref<InstanceType<typeof addPhraseComponent> | null>(null);
...
const openDrawer = async (data) => {
  ...
  jobPhraseDrawer.value?.showDrawerWithData(data);
};
const openAddDrawer = async () => {
  jobPhraseDrawer.value?.openDrawer();
};
