<template>
  <q-card class="no-shadow full-width">
    ...
    <q-table :columns="columns" :rows="memoListData" :loading="loading" row-key="id" v-model:pagination="pagination"
      hide-pagination>
...
const fetchMemoData = async () => {
  loading.value = true;
  const data = await memoStore.loadMemoData(props.clientId, props.clientFactoryId);
...
const showDeleteDialog = async (ids: string[]) => {
  $q.dialog({
    ...
    loading.value = true
    await memoStore.deleteMemo(ids, props.clientId, props.clientFactoryId);
    loading.value = false
    const data = await memoStore.loadMemoData(props.clientId, props.clientFactoryId)
    memoListData.value = data.map(row => {
      return { ...row, selected: false };
...
const onSubmit = async () => {
  loading.value = true;
  try {
    await memoStore.addNewMemo(props.clientId, props.clientFactoryId, data.value['content'] || '')
    loading.value = false;
    data.value = {};
    await fetchMemoData()
      ;
...
const onUpdate = async (index: number) => {
  try {
    ...
    loading.value = true;
    await memoStore.updateMemo(props.clientId, props.clientFactoryId, editableContect.value['content'], editableContect.value['id'])
