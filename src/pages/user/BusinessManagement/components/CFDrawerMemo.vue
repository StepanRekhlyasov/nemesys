<template>
  <q-card class="no-shadow full-width">
    ...
    <q-card-section class="q-pa-sm bg-grey-2 q-mt-none">
      <q-form ref="clientform" @submit="onSubmit">
...
const selectedMemo: Ref<ClientMemo[]> = ref([])
...
const loadMemoData = async () =>{
  loadData.value = true
  const q = query(collection(db, 'clients/' + props.clientId + '/memo'), where('deleted', '==', false));
  try{
...
async function onSubmit() {
  ...
  try {
    ...
    await addDoc(
      collection(db, 'clients/' + props.clientId + '/memo'),
      returnData
    )
...
async function onUpdate(index) {
  try {
    ...
    await updateDoc(
      doc(db, 'clients/' + props.clientId + '/memo/' + editableContect.value['id']),
      updateData
    );
...
async function deleteItem() {
  ...
  const ret = selectedMemo.value.map(async (memo) => {
      await updateDoc(
      docDb(db, 'clients/' + props.clientId + '/memo/' + memo.id),
      updateData
    );
  })
  Promise.all(ret)
