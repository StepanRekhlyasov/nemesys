      ...
      <q-card-section class=" q-pa-none">
        <q-table :columns="columns" :rows="state.BOList" row-key="id" selection="multiple" class="no-shadow"
          ...
          <template v-slot:body-cell-name="props">
            <q-td :props="props" class="q-pa-none">
              <div> {{ props.row['client_id'] ? applicantStore.state.clientList.find(client => client.id ===
                props.row['client_id'])?.name : '' }} </div>
              <div> {{ props.row['client_id'] && props.row['office_id'] ?
                applicantStore.state.clientList.find(client => client.id === props.row['client_id'])?.office?.find(
                  office => office.id === props.row['office_id'])?.name :
                '' }} </div>
            </q-td>
          </template>
        </q-table>
