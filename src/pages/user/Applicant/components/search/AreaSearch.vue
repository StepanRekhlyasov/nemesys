    ...
    if (docSnap.exists()) {
        ...
        for (var i = 0; i < region.length; i++) {
            ...
            for (var j = 0; j < pref.length; j++) {
                allPref.value.push(Object.keys(pref[j])[0] as never)
                allWards.value = [...allWards.value, ...pref[j][Object.keys(pref[j])[0]] as never]
            }
        }
    }
...
watch(
    ...
    (newVal, oldVal) => {
        ...
        emit('updateArea', selectedPrefectures, selectedWards);
    }
);
...
const onInputSubmit = () => {
    if (allPref.value.includes(searchInput.value as never)) {
        if (!prefectures.value.includes(searchInput.value as never)) {
            prefectures.value = [...prefectures.value, ...[searchInput.value as never]]
        }
        if (!searchKeyword.value.includes(searchInput.value as never)) {
            searchKeyword.value.push(searchInput.value as never)
        }
        searchInput.value = ''
    }
    if (allWards.value.includes(searchInput.value as never)) {
        if (!wards.value.includes(searchInput.value as never)) {
            wards.value.push(searchInput.value as never)
        }
        if (!searchKeyword.value.includes(searchInput.value as never)) {
            searchKeyword.value.push(searchInput.value as never)
        }
    ...
    <q-card class="no-shadow full-height q-pb-sm">
        ...
        <q-card class="no-shadow q-pl-md">
            <q-card-section>
                <div class="row">
                    <div class="col-6">
                        <q-input v-model:model-value="searchInput" v-bind="$attrs" outlined input-style="height: 36px;"
                            class="input" :placeholder="t('form.searchPlaceholder')" dense />
                    </div>
                ...
                <q-chip removable v-for="keyword in searchKeyword" @remove="removeSearchKeyword(keyword)"
                    :key="keyword">
                    {{ keyword }}
                </q-chip>
            </q-card-section>
