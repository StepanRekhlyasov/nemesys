...
const updateMediaOrder = async () => {
    console.log('media order changed');
}
...
<template>
    <q-card class="no-shadow full-width bg-grey-3">
        <q-card-section class="bg-grey-3">
            <div class="title">{{ t('menu.admin.mediaControl') }}</div>
        </q-card-section>
    </q-card>
        ...
        <draggable :list="mediaData" handle=".cursor_grab" @end="updateMediaOrder">
            <template #item="{ element, index }">
                <div class="row items-center">
                    ...
                    <q-popup-edit :validate="(val) => (val !== null && val !== '')" v-model="element.name" :cover="false"
                        :offset="[0, 10]" v-slot="scope" @save="updateMediaAux(element.id, element.name)">
