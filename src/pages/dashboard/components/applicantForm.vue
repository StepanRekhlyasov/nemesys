    ...
    <q-card style="width: 700px; max-width: 80vw;" class="q-pa-lg">
      <q-card-section>
        ...
        >
          ...
          <q-select outlined v-model="applicant['groupId']" :options="columns" label="Group" lazy-rules dense
            option-value="id"
            option-text="id"
            emit-value
            map-options
          ...
          <q-select label="Tag" outlined v-model="applicant['tag']" :options="tags" dense></q-select>


          <q-card-actions class="flex">
            <div class="col">
              <q-btn label="Cancel" color="negative" flat @click="$emit('close')" />
              <q-btn v-if="applicant['id']" label="Delete" class="q-ml-sm" color="negative" @click="$emit('onDelete', applicant)" />
            </div>
  ...
  props: {
   ...
   setup(props, { emit }) {
    ...
    const model = computed({
      get() { return props.applicantDialog }
    })

    const applicant = computed({
      get() { return props.applicantData }
    })
