  ...
  <q-page>
    ...
    <q-scroll-area style="height: 80vh; max-width: 90vw;">
      ...
      <div class="row no-wrap">
          <Column v-for="column in columns" :key="column.id" :column="column" :columns="columns" :tags="tags" />
      </div>
    </q-scroll-area>
    </div>
  </div>
...
export default {
  ...
  setup () {
    ...
    onMounted(() => {
      ...
      const q = query(collection(db, "kabanGroup"), where("deleted", "==", false));
      unsubscribe.value = onSnapshot(q, (snapshot) => {
        ...
        snapshot.docChanges().forEach((change) => {
          ...
          if (change.type === "added") {
            ...
            let index = columns.value.findIndex(x => x.id === change.doc.id);
            if (index > -1){
              columns.value[index] = {...{ id : change.doc.id }, ...change.doc.data()}
            }
          }
