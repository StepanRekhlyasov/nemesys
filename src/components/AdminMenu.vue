<template>
  <q-drawer v-model="leftDrawerOpen" show-if-above bordered behavior="desktop"
    style="background: linear-gradient(180deg, #4D0E4D 0%, #03101A 99.99%, #0C5180 100%);" :width="260" :mini="miniState">
    <q-list bordered padding class="rounded-borders text-white text-subtitle2 q-pa-none">
      <!-- <EssentialLink
        ...
        /> -->
      <EssentialLink v-for="parent in adminMenuParent" :key="$t(parent.title)" :active="parent.type === active_menu"
        v-bind="parent" @click="onChangeMenu(parent.type)" :admin="true" />
    </q-list>
  </q-drawer>

  <q-page-container class="bg-grey-1 flex">
    <template v-for="parent in adminMenuParent" :key="parent.title">
      <q-list class="admin-menu_slidebar q-pa-none" :class="{ 'active': parent.type == active_menu }">
        ...
        <template v-for="link in linksList" :key="link.link">
          <q-item
            ...
            class="admin-menu_slidebar_item q-pl-xl text-justify flex justify-between" :to="'/admin' + link.link" exact
            v-ripple clickable active-class="admin-menu_slidebar_class">
            <q-item-section>
              {{ $t(link.title) }}
            </q-item-section>
